...
<main class="main">
    <div class="news-item">
        ...
        <?php if (get_field('vklyuchitvyklyuchit_blok_drugie_akczii')) { ?>
            <div class="client">
                <div class="container">
                    <div class="client__in">
                        <h2 class="client__head section__title"><?php echo get_field('nazvanie_zagolovka_drugie_akczii'); ?></h2>
                        <div class="promo__in">
                            <div class="promo__body">

                                <?php
                                $items = get_field('drugie_akczii');
                                if ($items) {
                                    foreach ($items as $item) {
                                        setup_postdata($item);
                                ?>
                                        <a href="<?php echo get_permalink($item->ID); ?>" class="promo__body_item">
                                            ...
                                            <div class="promo__body_item_descr">
                                                <div class="promo__body_item_descr_head"><?php echo get_the_title($item->ID); ?></div>

                                                <?php if (get_field('vklyuchitvyklyuchit_datu', $item->ID)) { ?>
                                                    <div class="promo__body_item_descr_data"><?php echo get_field('data', $item->ID); ?></div>
                                                <?php } ?>

                                                <?php if (get_field('vklyuchitvyklyuchit_podzagolovok', $item->ID)) { ?>
                                                    <div class="promo__body_item_descr_text"><?php echo get_field('podzagolovok', $item->ID); ?></div>
                                                <?php } ?>
